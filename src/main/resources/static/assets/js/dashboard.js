...
function renderOrderCards() {
  ...
  orders.forEach((order) => {
    ...
    const { width, color } = getStatusStyle(order.status.toLowerCase());
    ...
    <div class="flex flex-row items-start py-4 gap-4 w-[327px] h-[130px]">
      <img src="/${firstProduct.img}" alt="Product Image" class="w-16 h-16 rounded-md object-cover" />
      <div class="flex-1 grid gap-1">
        <div class="flex flex-row items-start gap-1 w-[247px] h-[48px]">
          <h3 class="w-[247px] h-[48px] font-inter font-semibold text-[16px] leading-[24px] text-black flex-grow">${firstProduct.name}</h3>
        </div>
        <p class="text-xs text-[#7c7c7c] line-clamp-1">${firstProduct.description}</p>
        <div class="flex justify-between items-end py-1 w-[247px] h-[32px]">
          <div class="flex items-center">
            <span class="font-inter font-bold text-[16px] leading-[24px] text-black">${formatCurrency(firstProduct.price)}</span>
          </div>
    ...
    <span class="w-[50px] h-[17px] font-inter font-normal text-[14px] leading-[17px] text-[#797B7E]">${order.totalItems} Items</span>
    ...
    <div class="flex flex-row justify-end items-center gap-4 w-[327px] h-[48px]">
      <div class="flex flex-row items-center gap-1 w-[151px] h-[27px] flex-grow">
        <span class="w-[66px] h-[27px] font-inter font-bold text-[18px] leading-[27px] text-black">${formatCurrency(order.totalPrice)}</span>
      </div>
      <button class="flex flex-row justify-center items-center px-4 py-2 gap-2 w-[160px] h-[48px] bg-[#E2ECFF] rounded-[4.8px] ${getStatusButtonClass(order.status)}">
        <div class="w-[22px] h-[22px] flex-none order-0 grow-0">
          ${getStatusIcon(order.status)}
        </div>
        <div class="flex flex-col justify-center items-start p-0 ${width} h-[27px] bg-[rgba(255,255,255,0.01)] flex-none order-1 grow-0">
          <p class="h-[27px] font-inter font-bold text-[18px] leading-[27px] flex-none order-0 grow-0 ${color}">
            ${order.status}
          </p>
        </div>
      </button>
...
function getStatusStyle(status) {
  const statusStyle = {
    new: { width: 'w-[40px]', color: 'text-[#4079ED]' },
    cancelled: { width: 'w-[84px]', color: 'text-[#E31844]' },
    completed: { width: 'w-[84px]', color: 'text-[#12B76A]' },
    shipping: { width: 'w-[84px]', color: 'text-[#F04438]' },
  };

  return statusStyle[status.toLowerCase()] || "bg-gray-200 text-gray-700";
}
...
function getStatusButtonClass(status) {
  ...
  return statusButton[status.toLowerCase()] || "bg-[#e7efff] text-[#1570EF]";
}
