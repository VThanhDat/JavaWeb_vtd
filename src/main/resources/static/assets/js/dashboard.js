...
const items = {
  food: [
    {
      id: 1,
      title: "Cơm Gà Sốt Mắm",
      desc: "Cơm gà đùi góc tư sốt mắm tỏi + salad + canh",
      price: 40000,
      img: "/uploads/drink/87c2e5bb-93e5-4a2e-a805-9e10b9db9a41_caphesua.jpg",
    },
    {
      id: 2,
      title: "Cơm Gà Sốt Mắm",
  ...
  drink: [
    {
      id: 3,
      title: "S-Green Tea Macchiato",
      desc: "Lục Trà Macchiato - Size nhỏ",
      price: 51000,
      img: "/uploads/drink/87c2e5bb-93e5-4a2e-a805-9e10b9db9a41_caphesua.jpg",
    },
  ],
...
const orders = [
  {
    ...
    status: "New",
    products: [
      { type: "food", id: 1, quantity: 1 },
      { type: "drink", id: 3, quantity: 1 },
    ],
    customer: {
      name: "Nguyễn Văn A",
      phone: "039 3456 244",
      address: "Waseco Building, 10 Phổ Quang Street, Tân Bình, TP.HCM",
    },
    shippingFee: 20000,
  },
];
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderOrderCards() {
    ...
    orders.forEach((order) => {
      ...
      const product = items[firstProduct.type].find((p) => p.id === firstProduct.id);

      const totalProducts = order.products.reduce((sum, p) => sum + p.quantity, 0);
      let subtotal = 0;
      order.products.forEach((p) => {
        const prod = items[p.type].find((item) => item.id === p.id);
        if (prod) subtotal += prod.price * p.quantity;
      });
      const totalBill = subtotal + order.shippingFee;
      ...
      orderCard.innerHTML = `
        <!-- Order header -->
        <div class="flex flex-row justify-between items-start gap-1 w-[327px] h-[19px]">
          <span class="font-medium text-[#292929]">Order# ${order.id}</span>
          <span>${order.date}</span>
        </div>

        <!-- Product section -->
        <div class="flex flex-row items-start py-4 gap-4 w-[327px] h-[130px]">
          <img src="${product ? product.img : "/placeholder.svg"}" alt="Product Image" class="w-16 h-16 rounded-md object-cover" />
          <div class="flex-1 grid gap-1">
            <div class="flex flex-row items-start gap-1 w-[247px] h-[48px]">
              <h3 class="w-[247px] h-[48px] font-inter font-semibold text-[16px] leading-[24px] text-black flex-grow">${product ? product.title : "Unknown product"}</h3>
            </div>
            <p class="text-xs text-[#7c7c7c] line-clamp-1">${product ? product.desc : ""}</p>
            <div class="flex justify-between items-end py-1 w-[247px] h-[32px]">
              <div class="flex items-center">
                <span class="font-inter font-bold text-[16px] leading-[24px] text-black">${product ? formatCurrency(product.price) : "0 đ"}</span>
              </div>
              <span class="text-xs text-[#7c7c7c] font-normal">Quantity: ${firstProduct.quantity}</span>
            </div>
          </div>
        </div>

        <!-- Divider -->
        <hr class="w-[327px] border-t border-[#EFEFEF]" />

        <span class="w-[50px] h-[17px] font-inter font-normal text-[14px] leading-[17px] text-[#797B7E]">${totalProducts} Items</span>
        
        <div class="flex flex-row justify-end items-center gap-4 w-[327px] h-[48px]">
          <div class="flex flex-row items-center gap-1 w-[151px] h-[27px] flex-grow">
            <span class="w-[66px] h-[27px] font-inter font-bold text-[18px] leading-[27px] text-black">${formatCurrency(totalBill)}</span>
          </div>
          <button class="flex flex-row justify-center items-center px-4 py-2 gap-2 w-[160px] h-[48px] bg-[#E2ECFF] rounded-[4.8px] ${getStatusButtonClass(order.status)}">
      ...
      wrapper.className = "p-2 w-full sm:w-1/2";
      wrapper.appendChild(orderCard);
      orderListContainer.appendChild(wrapper);
    });
  }

  function getStatusButtonClass(status) {
    const statusMap = {
      new: "bg-[#e7efff] text-[#1570EF]",
      completed: "bg-[#e7f9ef] text-[#12B76A]",
      shipping: "bg-[#fef3f2] text-[#F04438]",
      cancelled: "bg-[#fef3f2] text-[#F04438]",
    };
    return statusMap[status.toLowerCase()] || "bg-[#e7efff] text-[#1570EF]";
  }

  function getStatusIcon(status) {
    const iconMap = {
      new: `<svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      cancelled: `<svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      shipping: `<svg width="23" height="22" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg">
